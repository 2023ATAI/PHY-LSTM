...
def main(cfg):
    ...
    ax[0].set_xlabel('observed SM (LSTM)')
    ax[0].set_ylabel('0d predicted SM (LSTM)')
    ...
    ax[1].set_title('PHYsLSTM Model')
    ax[1].set_title('(b)', loc='left')
    ax[1].set_xlabel('observed SM (PHYsLSTM)')
    ax[1].set_ylabel('0d predicted SM (PHYsLSTM)')
    metrics = 'y=' + '%.2f' % m + 'x' + '+ %.3f' % b
    ...
    colormap = plt.get_cmap("jet")
    Z1 = density_calc(observed_sm_mean, predicted_sm_mean, radius)
    ax[2].scatter(observed_sm_mean, predicted_sm_mean, c=Z1, s=sizes, cmap=colormap,
                  norm=colors.LogNorm(vmin=Z1.min(), vmax=Z1.max()))
    ax[2].plot(observed_sm_mean, regression_line, 'red', lw=0.8)  #####回归线
    ax[2].plot([0.05, 0.7], [0.05, 0.7], 'black', lw=0.8)  #####y=x
    ax[2].set_title('(c)', loc='left')
    ax[2].set_title('WBLSTM Model')
    ax[2].set_xlabel('observed SM (WBLSTM)')
    ax[2].set_ylabel('0d predicted SM (WBLSTM)')
    metrics = 'y=' + '%.2f' % m + 'x' + '+ %.3f' % b
    ax[2].text(0.05,0.67, metrics)
    print('y=' + '%.2f' % m + 'x' + '+ %.3f' % b)
